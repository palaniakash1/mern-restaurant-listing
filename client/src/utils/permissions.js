...
export const normalizePermissionOverrides = (role, requestedPermissions = {}) => {
  ...
  for (const [resource, actions] of Object.entries(requestedPermissions)) {
    ...
    const validActions = [...new Set(actions || [])].filter((action) =>
      allowedActions.includes(action)
    );

    if (validActions.length > 0) {
      normalized[resource] = validActions.sort();
    }
  }
...
export const hasPermission = (user, resource, action) =>
  Boolean(resolvePermissionsForUser(user)[resource]?.includes(action));

export const hasAnyPermission = (user, checks = []) =>
  checks.some(([resource, action]) => hasPermission(user, resource, action));
