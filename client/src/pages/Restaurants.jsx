...
import { HiSearch, HiLocationMarker, HiStar, HiArrowRight, HiClock } from 'react-icons/hi';
...
export default function Restaurants() {
  ...
  useEffect(() => {
    const filtered = allRestaurants.filter((restaurant) => {
      ...
      const matchesSearch = !q ||
        ...
        (restaurant.categories || []).some((c) => c.toLowerCase().includes(q.toLowerCase()));
  ...
  const categories = useMemo(() => {
    return [...new Set(allRestaurants.flatMap((r) => r.categories || [r.category]).filter(Boolean))];
  }, [allRestaurants]);

  const getOpeningStatus = (restaurant) => {
    if (!restaurant.openingHours || !restaurant.openingHours.length) return null;
    const now = new Date();
    const day = now.toLocaleDateString('en-US', { weekday: 'long' });
    const todayHours = restaurant.openingHours.find(h => h.day === day);
    if (!todayHours || todayHours.closed) return { status: 'Closed', color: '!text-[#bf1e18]' };
    ...
    if (currentTime >= openTime && currentTime < closeTime) {
      if (closeTime && new Date(`1970-01-01 ${closeTime}`).getTime() - new Date(`1970-01-01 ${currentTime}`).getTime() < 3600000) {
        ...
        <div className="max-w-7xl mx-auto px-6 -mt-16 relative z-20">
          <div className="!bg-white rounded-2xl shadow-2xl p-6 md:p-8">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="h-14 !bg-[#fff1f0] rounded-full animate-pulse" />
              ))}
            </div>
          </div>
          <div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3, 4, 5, 6].map((i) => (<SkeletonCard key={i} variant="restaurant" />))}
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="!bg-[#fff8f7] min-h-screen flex items-center justify-center">
        <div className="text-center max-w-md p-8 !bg-white rounded-2xl shadow-2xl">
          ...
          <h3 className="text-2xl font-bold !text-[#201a1a] mb-2 font-[Manrope]">Failed to Load Restaurants</h3>
      ...
      <header className="relative pt-32 pb-20 px-6 overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-3xl mx-auto">
          ...
          <p className="!text-white/80 text-lg md:text-xl font-[Inter]">
            Discover handpicked restaurants where every dish is thoughtfully crafted
          </p>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 py-16">
          ...
          return (
            <div className="mb-16">
              <div className="flex items-center gap-2 mb-8">
                <HiStar className="w-6 h-6 !text-[#bf1e18]" fill="currentColor" />
                <span className="text-xs font-bold uppercase tracking-wider !text-[#bf1e18]">Featured Selection</span>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className="relative h-72 overflow-hidden">
                        ...
                        {openingStatus && (
                          <div className={`absolute top-4 right-4 px-3 py-1 rounded-full !bg-white/90 text-xs font-bold ${openingStatus.color}`}>
                        ...
                        <div className="absolute bottom-4 right-4 !bg-white rounded-xl p-1 shadow-lg">
                          <img src={restaurant.imageLogo} alt={`${restaurant.name} logo`} className="w-16 h-16 rounded-lg object-cover" />
                        </div>
                      </div>
                      <div className="p-6">
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <div className="flex items-center gap-1 ml-auto">
                            <HiStar className="w-4 h-4 !text-yellow-500" fill="currentColor" />
              ...
              >
                <div className="h-52 overflow-hidden relative rounded-t-xl">
                  ...
                  <div className="absolute bottom-3 right-3 !bg-white rounded-lg p-0.5 shadow-lg">
                    <img src={restaurant.imageLogo} alt={`${restaurant.name} logo`} className="w-10 h-10 rounded-md object-cover" />
                  </div>
                </div>
                <div className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {openingStatus && (
                      <span className={`text-[10px] font-bold ${openingStatus.color}`}>
                        {openingStatus.status}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1 !text-[#534342] text-xs">
                      <HiStar className="w-3.5 h-3.5 !text-yellow-500" fill="currentColor" />
                      <span className="font-bold">{restaurant.rating || 'New'}</span>
                      <span className="!text-[#534342]/70">({restaurant.reviewCount || 0})</span>
                    </div>
                    <div className="flex items-center gap-1 !text-[#534342] text-xs">
        ...
        {restaurants.length === 0 && (
          <div className="text-center py-20">
            ...
            <h3 className="text-2xl font-bold !text-[#201a1a] mb-2 font-[Manrope]">No Restaurants Found</h3>
            <p className="!text-[#534342] max-w-md mx-auto">Try adjusting your search or filters to discover more dining options.</p>
          </div>
        )}
      </main>
    </div>
